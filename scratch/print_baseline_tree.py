...
def main():
    ...
    X = df_features[baseline_features]
    ...
    # 1. Depth-3 Decision Tree on Baseline Features
    dt3 = DecisionTreeClassifier(max_depth=3, min_samples_split=2, random_state=42)
    dt3.fit(X, y_enc)
    
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    scores3 = cross_val_score(dt3, X, y_enc, cv=cv, scoring='accuracy')
    
    print("\n=================== BASELINE FEATURES DT DEPTH 3 ===================")
    ...
    print("\nTree Structure:")
    print(export_text(dt3, feature_names=baseline_features))
    print("\nClassification Report:")
    print(classification_report(y_enc, dt3.predict(X), target_names=class_names))

    # 2. Depth-4 Decision Tree on Baseline Features
    dt4 = DecisionTreeClassifier(max_depth=4, min_samples_split=2, random_state=42)
    dt4.fit(X, y_enc)
    scores4 = cross_val_score(dt4, X, y_enc, cv=cv, scoring='accuracy')
    
    print("\n=================== BASELINE FEATURES DT DEPTH 4 ===================")
    ...
    print("\nTree Structure:")
    print(export_text(dt4, feature_names=baseline_features))
    print("\nClassification Report:")
