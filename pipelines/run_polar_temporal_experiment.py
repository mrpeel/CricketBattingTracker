#!/usr/bin/env python3
"""
28-Channel Dual-Hand Polar Self-Referential Temporal Dynamics Experiment (Option A)
===================================================================================
Tests whether keeping watch data as is (Earth-aligned with sensor fusion quaternions)
and replacing raw unaligned Polar axes with rotation-invariant magnitudes and
self-referential temporal dynamics over time:
  1. p_gyro_mag      - Total rotational speed
  2. p_acc_mag       - Total linear acceleration
  3. p_gyro_d25ms    - Instantaneous angular jerk (26ms)
  4. p_gyro_d100ms   - Downswing acceleration ramp (100ms)
  5. p_acc_d25ms     - Linear strike jerk (26ms)
  6. p_rel_surge     - Relative speed surge over 100ms
improves stroke classification accuracy (specifically Power Drive vs Slog)
on our authoritative 4-session holdout scorecard.
"""
...
from telemetry_engine import (
    load_parquet_session,
    BatPlaneGeometryThreeFamilyTCN,
    evaluate_multitier_scorecard,
    normalise_shot_type,
    CLASSES,
    HOLDOUT_SESSIONS,
...
EXP_STATS_PATH = os.path.join(PIPELINES_DIR, "tcn_polar_temporal_norm_stats.json")
EXP_MODEL_PT_PATH = os.path.join(PIPELINES_DIR, "tcn_polar_temporal_model.pt")
EXP_REPORT_OUT = os.path.join(ROOT_DIR, "polar_temporal_experiment_scorecard.md")

# 19 Watch channels (Sensor + Earth-aligned + Gravity + Quaternions)
WATCH_FEATURES = [
...
# 6 Rotation-Invariant Self-Referential Polar Temporal Channels
# (Replaces unaligned raw p_acc_x,y,z and p_gyro_x,y,z)
POLAR_TEMPORAL_FEATURES = [
    'p_gyro_mag',      # Instantaneous rotational speed
    'p_acc_mag',       # Instantaneous linear acceleration
    'p_gyro_d25ms',    # Instantaneous angular jerk (26ms)
    'p_gyro_d100ms',   # Downswing acceleration ramp (100ms)
    'p_acc_d25ms',     # Linear strike jerk (26ms)
    'p_rel_surge',     # Relative speed surge over 100ms
]
...
FEATURES_OPTION_A = WATCH_FEATURES + POLAR_TEMPORAL_FEATURES + ADDITIONAL_FEATURES
NUM_FEATURES = len(FEATURES_OPTION_A)
CLASS_TO_IDX = {c: i for i, c in enumerate(CLASSES)}

DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
WINDOW_LEN = 2048
BATCH_SIZE = 32
WEIGHT_FAM = torch.tensor([1.0, 1.0, 1.0], device=DEVICE)
WEIGHT_2A = torch.tensor([1.0, 2.0, 1.0, 1.0], device=DEVICE)

# Bat Plane Geometry Tensors:
...
def sync_unified_dataset():
    ...
    live_sessions = sorted([os.path.basename(p) for p in glob.glob(pattern) if os.path.isdir(p)])
    newly_built = 0
    for s in live_sessions:
        out_parquet = os.path.join(DATASET_DIR, f"{s}_unified.parquet")
        raw_narr_path = os.path.join(SESSIONS_DIR, s, "narrations_raw.json")
        raw_watch_path = os.path.join(SESSIONS_DIR, s, "WatchAccelerometer.bin.gz")
...
def load_dataset_for_training(session_name):
    ...
    has_polar = df['has_polar'].values if 'has_polar' in df else np.zeros(len(df), dtype=np.float32)
    p_gyro_mag = df['p_gyro_mag'].values if 'p_gyro_mag' in df else np.zeros(len(df), dtype=np.float32)
    p_acc_mag = df['p_acc_mag'].values if 'p_acc_mag' in df else np.zeros(len(df), dtype=np.float32)
    
    df['p_gyro_mag'] = p_gyro_mag.astype(np.float32)
    df['p_acc_mag'] = p_acc_mag.astype(np.float32)
    
    # Compute self-referential temporal differences
    # 423 Hz: 11 frames = ~26ms, 42 frames = ~100ms
    d25 = np.zeros_like(p_gyro_mag)
    d25[11:] = (p_gyro_mag[11:] - p_gyro_mag[:-11]) * has_polar[11:]
    df['p_gyro_d25ms'] = d25.astype(np.float32)
    
    d100 = np.zeros_like(p_gyro_mag)
    d100[42:] = (p_gyro_mag[42:] - p_gyro_mag[:-42]) * has_polar[42:]
    df['p_gyro_d100ms'] = d100.astype(np.float32)
    
    d25a = np.zeros_like(p_acc_mag)
    d25a[11:] = (p_acc_mag[11:] - p_acc_mag[:-11]) * has_polar[11:]
    df['p_acc_d25ms'] = d25a.astype(np.float32)
    
    surge = np.zeros_like(p_gyro_mag)
    surge[42:] = ((p_gyro_mag[42:] - p_gyro_mag[:-42]) / (p_gyro_mag[:-42] + 1.0)) * has_polar[42:]
    df['p_rel_surge'] = surge.astype(np.float32)
    
    X = df[FEATURES_OPTION_A].fillna(0.0).values.astype(np.float32)
...
def evaluate_holdout_candidate_metrics(model, holdout_shot_windows):
    ...
    with torch.no_grad():
        for i in range(0, len(all_x), batch_size):
            ...
            c_idx = WINDOW_LEN // 2
            w_s = max(0, c_idx - 42)
            w_e = min(WINDOW_LEN, c_idx + 42)
            for b in range(len(xb)):
                p = probs[b, :, w_s:w_e]
                shot_probs = p[2:len(CLASSES), :].max(axis=1)
                top_rel_idx = np.argmax(shot_probs)
                pred_c = top_rel_idx + 2
                y_pred.append(pred_c)
                
    shot_acc = accuracy_score(y_true, y_pred)
    macro_f1 = f1_score(y_true, y_pred, average="macro", zero_division=0)
    return shot_acc, macro_f1
...
def train_and_select_checkpoint(train_data, holdout_data, train_sessions):
    ...
    for s_idx, (X, y, df) in enumerate(train_data):
        n_frames = len(X)
        if n_frames < window_len:
            continue
        for i in range(0, n_frames - window_len, stride):
            yw = y[i:i+window_len]
            if (yw >= 2).any():
    ...
    print(f"Loaded {len(train_dataset)} training windows from {len(train_sessions)} physical sessions.", flush=True)
    ...
    print("="*95, flush=True)
    print("🚀 28-CHANNEL POLAR SELF-TEMPORAL TCN TRAINING (EPOCHS 1 TO 35)", flush=True)
    print(f"   Input Channels: {NUM_FEATURES} (19 Watch + 6 Polar Self-Temporal + 3 Metadata)")
    print("   Polar Channels: [p_gyro_mag, p_acc_mag, p_gyro_d25ms, p_gyro_d100ms, p_acc_d25ms, p_rel_surge]")
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        ...
        if ho_macro_f1 > best_macro_f1:
            ...
            torch.save(model.state_dict(), EXP_MODEL_PT_PATH)
        else:
            if epoch >= 6:
                patience_counter += 1
...
def main():
    logger = setup_training_logger(prefix="polar_temporal_experiment")
    parser = argparse.ArgumentParser(description="28-Channel Dual-Hand Polar Self-Referential Temporal Dynamics Experiment")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility (default: 42)")
    args = parser.parse_args()

    print("="*100, flush=True)
    print("  28-CHANNEL POLAR SELF-REFERENTIAL TEMPORAL DYNAMICS EXPERIMENT (OPTION A)", flush=True)
    ...
    # Critical: Enforce unit variance (mad=1.0) on zero-filled Polar channels to prevent amplification
    zero_filled_indices = [
        FEATURES_OPTION_A.index(c) for c in FEATURES_OPTION_A
        if c.startswith('p_') or c == 'has_polar'
    ]
    for idx in zero_filled_indices:
        mad[idx] = 1.0
    
    stats_data = {'features': FEATURES_OPTION_A, 'classes': CLASSES, 'median': med.tolist(), 'mad': mad.tolist()}
    with open(EXP_STATS_PATH, 'w') as f:
        json.dump(stats_data, f, indent=2)
    print(f"✅ Saved 28-channel feature normalization stats -> {EXP_STATS_PATH}")
    ...
    print("\n" + "="*115)
    print("📊 28-CHANNEL POLAR SELF-TEMPORAL EXPERIMENT SCORECARD (OPTION A)")
    ...
    report = f"""# 28-Channel Polar Self-Referential Temporal Dynamics Experiment Scorecard Report (Option A)
...
"""
    ...
    with open(EXP_REPORT_OUT, 'w') as f:
        f.write(report)
        
    print(f"\n✅ Saved experimental scorecard report to {EXP_REPORT_OUT}")
