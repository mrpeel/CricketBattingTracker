#!/usr/bin/env python3
"""
34-Channel Dual-Hand Polar Self-Referential Temporal Dynamics Experiment (Option B)
===================================================================================
Tests whether keeping watch data as is (Earth-aligned with sensor fusion quaternions)
AND retaining raw Polar 3-axis signals (for 3D trajectory curvature / pronation vs flexion)
WHILE appending rotation-invariant magnitudes and self-referential temporal dynamics:
...
EXP_STATS_PATH = os.path.join(PIPELINES_DIR, "tcn_polar_option_b_norm_stats.json")
EXP_MODEL_PT_PATH = os.path.join(PIPELINES_DIR, "tcn_polar_option_b_model.pt")
EXP_REPORT_OUT = os.path.join(ROOT_DIR, "polar_temporal_option_b_scorecard.md")
...
# Option B: 28 Baseline Features + 6 Polar Self-Temporal Features = 34 Features
FEATURES_OPTION_B = FEATURES + POLAR_TEMPORAL_FEATURES
NUM_FEATURES = len(FEATURES_OPTION_B)
...
def load_dataset_for_training(session_name):
    ...
    X = df[FEATURES_OPTION_B].fillna(0.0).values.astype(np.float32)
...
def train_and_select_checkpoint(train_data, holdout_data, train_sessions):
    ...
    print("="*95, flush=True)
    print("🚀 34-CHANNEL POLAR OPTION B TCN TRAINING (EPOCHS 1 TO 35)", flush=True)
    print(f"   Input Channels: {NUM_FEATURES} (28 Baseline Channels + 6 Polar Self-Temporal Channels)")
    print("   Polar Channels: [p_acc_x,y,z, p_gyro_x,y,z, p_gyro_mag, p_acc_mag, p_gyro_d25ms, p_gyro_d100ms, p_acc_d25ms, p_rel_surge]")
...
def main():
    logger = setup_training_logger(prefix="polar_option_b_experiment")
    parser = argparse.ArgumentParser(description="34-Channel Dual-Hand Polar Option B Experiment")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility (default: 42)")
    args = parser.parse_args()

    print("="*100, flush=True)
    print("  34-CHANNEL POLAR OPTION B EXPERIMENT (3D VECTORS + SELF-TEMPORAL DYNAMICS)", flush=True)
    ...
    zero_filled_indices = [
        FEATURES_OPTION_B.index(c) for c in FEATURES_OPTION_B
        if c.startswith('p_') or c == 'has_polar'
    ]
    for idx in zero_filled_indices:
        mad[idx] = 1.0
    
    stats_data = {'features': FEATURES_OPTION_B, 'classes': CLASSES, 'median': med.tolist(), 'mad': mad.tolist()}
    with open(EXP_STATS_PATH, 'w') as f:
        json.dump(stats_data, f, indent=2)
    print(f"✅ Saved 34-channel feature normalization stats -> {EXP_STATS_PATH}")
    ...
    print("\n" + "="*115)
    print("📊 34-CHANNEL POLAR EXPERIMENT SCORECARD (OPTION B)")
    ...
    report = f"""# 34-Channel Polar Self-Referential Temporal Dynamics Experiment Scorecard Report (Option B)
