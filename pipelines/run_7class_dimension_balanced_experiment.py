#!/usr/bin/env python3
"""
pipelines/run_7class_dimension_balanced_experiment.py — Refined Bat Plane Geometry 3-Family TCN (Head 2A 144d Triplet + Loss Calibration)

Refinements:
  - Head 2A (Vertical-Bat Sub-Head):
    * Multi-Scale Feature Triplet: [Pool(L5) [16d], Pool(L7) [64d], Proj(L10) [64d]] (144 dims)
    * Dense Classifier: Linear(144, 64) -> BatchNorm1d -> GELU -> Dropout(0.1) -> Linear(64, 4)
    * Target Classes: [DRIVE/DEFENCE, POWER DRIVE, GLANCE/FLICK, DEFLECTION/GUIDE]
    * Intra-Head Sub-Loss Weights: weight_2a = [1.0, 1.4, 1.0, 1.0] (label_smoothing=0.1)
  - Head 2B (Cross-Bat Sub-Head):
    * Multi-Scale Feature Triplet: [Pool(L5) [16d], Pool(L7) [64d], Proj(L10) [64d]] (144 dims)
    * Dense Classifier: Linear(144, 64) -> BatchNorm1d -> GELU -> Dropout(0.1) -> Linear(64, 2)
    * Target Classes: [PULL/HOOK/SLOG, CUT/PUNCH] (Unweighted label_smoothing=0.1)
  - Head 2C (Floor / Crouch): SWEEP (Direct passthrough)

Optimization & Checkpointing:
  - AdamW (weight_decay=1e-2), Discriminative LR (3e-4 L1-5, 1e-3 L6-10+Heads, 3-epoch warmup)
  - Peak Holdout Candidate Macro-F1 Checkpointing (patience=10, min_delta=0.0)
  - Telemetry Engine Multi-Tier Scorecard across all 59 physical sessions.
"""
...
# Targeted Intra-Head Sub-Loss Calibration for Head 2A (Vertical-Bat: [DRIVE/DEFENCE, POWER DRIVE, GLANCE/FLICK, DEFLECTION/GUIDE])
...
# =============================================================================
# Refined Bat Plane Geometry 3-Family Multi-Head TCN Architecture
# =============================================================================
...
def main():
    ...
    print("  REFINED BAT PLANE GEOMETRY 3-FAMILY TCN (HEAD 2A 144D TRIPLET + LOSS CALIBRATION)", flush=True)
    ...
    print("="*80, flush=True)
    print("🚀 TRAINING REFINED BAT PLANE GEOMETRY 3-FAMILY TCN", flush=True)
    print(f"   Family 0 (Vertical-Bat, 4 Classes): [DRIVE/DEFENCE, POWER DRIVE, GLANCE/FLICK, DEFLECTION/GUIDE] (144d Triplet, weight_2a={WEIGHT_2A.cpu().tolist()})")
    print(f"   Family 1 (Cross-Bat, 2 Classes):    [PULL/HOOK/SLOG, CUT/PUNCH] (144d Triplet)")
    print(f"   Family 2 (Floor / Crouch, 1 Class): [SWEEP] (Passthrough)")
    ...
    print("="*100, flush=True)
    print(f"📊 EVALUATING ACROSS ALL {len(all_parquet_sessions)} PHYSICAL SESSIONS VIA TELEMETRY ENGINE (7-CLASS REFINED BAT PLANE)...", flush=True)
    print("="*100, flush=True)
    metrics = evaluate_7class_scorecard(all_parquet_sessions, model, stats_data, device=DEVICE)
    
    print("\n" + "="*115, flush=True)
    print("🏆 FINAL REFINED 7-CLASS BAT PLANE MULTI-TIER SCORECARD (144D TRIPLET HEAD 2A + 1.4X LOSS)", flush=True)
    ...
    print("\n" + "="*115, flush=True)
    print("🎯 HOLDOUT 7-CLASS ACCURACY BREAKDOWN (REFINED BAT PLANE GEOMETRY)", flush=True)
    print("="*115, flush=True)
    ...
    out_model_path = os.path.join(PIPELINES_DIR, "tcn_7class_bat_plane_refined_model.pt")
    torch.save(best_model_state, out_model_path)
    print(f"\n💾 Saved Refined Bat Plane 7-class model checkpoint to: {out_model_path}\n", flush=True)
