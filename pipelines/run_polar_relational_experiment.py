#!/usr/bin/env python3
"""
31-Channel Dual-Hand Relational TCN Experiment
==============================================
Tests whether adding 3 rotationally invariant scalar relational channels:
  1. rel_torque_ratio = (p_gyro_mag / (w_gyro_mag + 1.0)) * has_polar
  2. rel_force_ratio  = (p_acc_mag / (w_acc_mag + 1.0)) * has_polar
  3. rel_diff_energy  = (w_gyro_mag - p_gyro_mag) * has_polar
improves stroke classification accuracy (specifically Power Drive vs Slog) on our authoritative holdout scorecard.
"""
...
EXP_STATS_PATH = os.path.join(PIPELINES_DIR, "tcn_relational_norm_stats.json")
EXP_MODEL_PT_PATH = os.path.join(PIPELINES_DIR, "tcn_polar_relational_model.pt")
EXP_REPORT_OUT = os.path.join(ROOT_DIR, "polar_relational_experiment_scorecard.md")

# 31 Features: 28 baseline features + 3 frame-independent relational features
RELATIONAL_FEATURES = [
    "rel_torque_ratio",
    "rel_force_ratio",
    "rel_diff_energy"
]
FEATURES_31 = FEATURES + RELATIONAL_FEATURES
NUM_FEATURES = len(FEATURES_31)
...
def load_dataset_for_training(session_name):
    ...
    # Compute the 3 relational channels on the fly
    has_polar = df['has_polar'].values if 'has_polar' in df else np.zeros(len(df), dtype=np.float32)
    w_gyro_mag = df['w_gyro_mag'].values if 'w_gyro_mag' in df else np.linalg.norm(df[['w_gyro_x', 'w_gyro_y', 'w_gyro_z']].values, axis=1)
    p_gyro_mag = df['p_gyro_mag'].values if 'p_gyro_mag' in df else np.zeros(len(df), dtype=np.float32)
    w_acc_mag = df['w_acc_mag'].values if 'w_acc_mag' in df else np.linalg.norm(df[['w_acc_x', 'w_acc_y', 'w_acc_z']].values, axis=1)
    p_acc_mag = df['p_acc_mag'].values if 'p_acc_mag' in df else np.zeros(len(df), dtype=np.float32)
    
    df['rel_torque_ratio'] = (p_gyro_mag / (w_gyro_mag + 1.0) * has_polar).astype(np.float32)
    df['rel_force_ratio']  = (p_acc_mag / (w_acc_mag + 1.0) * has_polar).astype(np.float32)
    df['rel_diff_energy']  = ((w_gyro_mag - p_gyro_mag) * has_polar).astype(np.float32)
    
    X = df[FEATURES_31].fillna(0.0).values.astype(np.float32)
...
def train_and_select_checkpoint(train_data, holdout_data, train_sessions):
    ...
    print("="*95, flush=True)
    print("🚀 31-CHANNEL DUAL-HAND RELATIONAL TCN TRAINING (EPOCHS 1 TO 35)", flush=True)
    print("   Input Channels: 31 (28 baseline + 3 scalar relational channels)")
...
def main():
    logger = setup_training_logger(prefix="polar_relational_experiment")
    parser = argparse.ArgumentParser(description="31-Channel Dual-Hand Relational TCN Experiment")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility (default: 42)")
    args = parser.parse_args()

    print("="*100, flush=True)
    print("  31-CHANNEL DUAL-HAND RELATIONAL TCN EXPERIMENT", flush=True)
    ...
    # Critical: Enforce unit variance (mad=1.0) on zero-filled Polar channels and relational channels
    zero_filled_indices = [
        FEATURES_31.index(c) for c in FEATURES_31
        if c.startswith('p_') or c == 'has_polar' or c.startswith('rel_')
    ]
    for idx in zero_filled_indices:
        mad[idx] = 1.0
    
    stats_data = {'features': FEATURES_31, 'classes': CLASSES, 'median': med.tolist(), 'mad': mad.tolist()}
    with open(EXP_STATS_PATH, 'w') as f:
        json.dump(stats_data, f, indent=2)
    print(f"✅ Saved 31-channel feature normalization stats -> {EXP_STATS_PATH}")
    ...
    print("\n" + "="*115)
    print("📊 31-CHANNEL DUAL-HAND RELATIONAL TCN EXPERIMENT SCORECARD")
    ...
    report = f"""# 31-Channel Dual-Hand Relational TCN Experiment Scorecard Report
